...
import { getFolderDisplayName } from "@yomika/shared";
import {
  type FolderFieldsFragment,
...
function FolderRenameContent({
  ...
  folder: FolderFieldsFragment;
  onClose: () => void;
  onCompleted: FolderRenameDialogProps["onCompleted"];
}) {
  const [name, setName] = useState(getFolderDisplayName(folder.path));
  const [nameError, setNameError] = useState<string | undefined>();
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const [updateFolder, updateState] = useMutation(UpdateFolderDocument);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await onCompleted?.(result.data?.updateFolder.path);
      onClose();
    } catch (error) {
